...
class ChatConsumer(AsyncJsonWebsocketConsumer):
    async def connect(self):
        ...
        # (you can't just change self.channel_name 
        # to send directly to channel, that doesn't work)
    ...
    async def receive_json(self, message):
        ...
        #     created: message creation date that calculated on client 
        #              (e.g. 2016-03-14 14:20:43.393)
        # }
    ...
    def create_message(self, message):
        ...
        sender_profile = Profile.objects.get(
            user__id = self.scope['user'].id)

        receiver_profile = Profile.objects.get(
            user__id = message['receiver'])

        try:
            all_user_chats = Chat.objects.filter(
                users__user__id = self.scope["user"].id)

            chat = all_user_chats.get(
                users__user__id = int(message['receiver']))
